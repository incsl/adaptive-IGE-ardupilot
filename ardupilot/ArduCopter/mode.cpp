...
*/
        if(navigating)
        {
            float max_descent_speed = abs(g.land_speed)*0.5f;
            // float land_slowdown = MAX(0.0f, pos_control->get_horizontal_error()*(max_descent_speed/precland_acceptable_error));
            cmb_rate = -max_descent_speed;//MIN(-precland_min_descent_speed, -max_descent_speed+land_slowdown);
        }
    }
...
void Mode::land_run_sine_control(bool pause_descent)
{
    const bool navigating = pos_control->is_active_xy();
    float cmb_rate = 0;
    float curr_alt = pos_control->get_real_alt();
    _real_alt = curr_alt;
    ...
    // if ((curr_alt<=44.0f))
    {
        ...
        // Constrain the demanded vertical velocity so that it is between the configured maximum descent speed and the configured minimum descent speed.
        cmb_rate = constrain_float(cmb_rate, max_land_descent_velocity, -abs(g.land_speed));
        _angle += 0.00314f/2.0f;
        _angle = wrap_PI(_angle);
        if(navigating)
        {   
            float max_descent_speed = abs(g.land_speed)*1.0f;
            cmb_rate = -1.5f*0.248f*max_descent_speed*sinf(_angle); // case of  30cm amplitude
            // cmb_rate = -0.248f*max_descent_speed*sinf(_angle); // case of  20cm amplitude
        }
    }
    }
    // if ((curr_alt<=12.0f))
