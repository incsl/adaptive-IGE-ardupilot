...
//Adaptive parameter
# define K1 3.2f //3.2f
# define K2 3.95f //3.95f
# define K3 1.5f //1.5f
# define H_para 0.252015625e-3f
...
void AC_PosControl::init_adap()
{    
    _rho_hat_1 = _rho_init;
    _rho_hat_2 = _rho_init;
    _home_alt = _inav.get_altitude();
}
...
{   
    float curr_alt = _inav.get_altitude();
    float v_bar = 0.0f;
    float v_bar_dot = 0.0f;
    const float m = 0.7f; // Quadrotor weight
    _limit.pos_up = false;
    ...
    // Calculate integration of position error
    _chi += _pos_error.z*_dt;
    _chi = constrain_float(_chi,-_chi_max,_chi_max);
    ...
    if (K1*_pos_error.z < _speed_down_cms) {
        _pos_error.z  = _speed_down_cms/K1;
        _limit.vel_down = true;
    }
    if (K1*_pos_error.z> _speed_up_cms) {
        _pos_error.z = _speed_up_cms/K1;
        _limit.vel_up = true;
    }
    ...
    if (_flags.reset_rate_to_accel_z) {
        _vel_last.z = _vel_desired.z;
    }

    // feed forward desired acceleration calculation
    if (_dt > 0.0f) {
        if (!_flags.freeze_ff_z) {
            _accel_desired.z = (_vel_desired.z - _vel_last.z) / _dt;
        } else {
    ...
    if (_flags.reset_rate_to_accel_z) {
        ...
        _vel_error.z = _vel_error_filter.apply(_vel_desired.z - curr_vel.z, _dt);
    }
    float z_accel_meas;         // actual acceleration

    // Calculate Earth Frame Z acceleration
    z_accel_meas = -(_ahrs.get_accel_ef_blended().z + GRAVITY_MSS) * 100.0f;
    // Calculate virtual control input of backstepping controller
    v_bar = _vel_desired.z + K1*_pos_error.z + K2*_chi;
    v_bar_dot = _accel_desired.z - z_accel_meas + K1*_vel_error.z  + K2*_pos_error.z; //



    //--------Robust control fractional model------------//
    float alt = (curr_alt - _home_alt)/100.0f;
    float h_dot_2 = 0.0f;
    _h = H_para/sq(alt+0.05);
    h_dot_2 = -(0.01f*_h*curr_vel.z)/(alt+0.05);
    float lambda_1 =0.0f;
    lambda_1 = -(_h*(_motors.get_throttle_hover()*1428.57f + v_bar_dot) + h_dot_2*(v_bar - curr_vel.z))*(v_bar - curr_vel.z);
    float beta_dot =0.0f;
    float ub=0.0f;
    beta_dot = -100.0f*(v_bar_dot+_motors.get_throttle_hover()*1428.57f)*(v_bar - curr_vel.z);
    _beta_d += beta_dot*_dt;
    float beta_max = abs(_rho_hat_1*_h);

    _beta_d = constrain_float(_beta_d,-0.025*beta_max ,0.025*beta_max); 
    ub = _beta_d*(v_bar_dot+_motors.get_throttle_hover()*1428.57f) + 0.00175f*beta_max*(v_bar - curr_vel.z);
    _rho_hat_1 += 100.0f*lambda_1*_dt;
    _rho_hat_1 = constrain_float(_rho_hat_1,4.5f,9.5f);
    float u=0.0f;
    u = m*((1-_rho_hat_1*_h)*(v_bar_dot+_motors.get_throttle_hover()*1428.57f)+(K3 -_rho_hat_1*h_dot_2)*(v_bar - curr_vel.z) + _pos_error.z)+ ub;
    //---------------------MRAC--------------------------------//
    // float alt = (curr_alt - _home_alt);
    // float u_a = 0.0f;
    // float u=0.0f;
    // float e_1 = 0.0f;
    // float e_2 = 0.0f;
    // float lambda_1 =0.0f;
    // float lambda_2 =0.0f;
    // float state1 = _varphi_1;
    // float state2= _varphi_2;
    // _varphi_1 += (state2+0.1947f*(_pos_target.z- _home_alt))*_dt;
    // _varphi_2 += (-5.6861f*state1-0.5264f*state2+5.6588f*(_pos_target.z- _home_alt))*_dt;
    // e_1 = alt-_varphi_1;
    // e_2 = (alt-_varphi)/_dt-_varphi_2;
    // lambda_2 =(0.15f*e_1+0.01f*e_2);
    // lambda_1 = alt*lambda_2/100;
    // _rho_hat_1 += lambda_1*_dt;
    // _rho_hat_2 += lambda_2*_dt;
    //     _rho_hat_1 = constrain_float(_rho_hat_1,1.5,0);
    //     _rho_hat_2 = constrain_float(_rho_hat_2,1.0f,2.0f);
    // u_a = 40*( - _rho_hat_1*alt/100 - _rho_hat_2);
    // if (alt<20)
    // {
    // u_a = constrain_float(u_a,-100.0f,100.0f);
    // }
    // else
    // {
    //     u_a = 0;
    // }
    //  u = m*((v_bar_dot+_motors.get_throttle_hover()*1428.57f)+K3*(v_bar - curr_vel.z) + _pos_error.z);//+u_a;
    //--------------------------------------------------------//
    //--------Adaptive control exponential model-------//
    // _h = expf(-1.3f*alt/0.0635f);
    // _varphi_1 = expf(-2.5f*alt/0.0635f);
    // _varphi_2 = expf(-1.5f*alt/0.0635f); 
    // float lambda_1 =0.0f;  
    // float lambda_2 =0.0f;
    // _u = m*(K3*(v_bar - curr_vel.z)+(v_bar_dot+_motors.get_throttle_hover()*1428.57f)+ _pos_error.z);
    // _u = _u/(1+1.0f*(_rho_hat_1*_varphi_1 + _rho_hat_2*_varphi_2));
    // _varphi = (_rho_hat_1*_varphi_1 + _rho_hat_2*_varphi_2);

    // lambda_1 = -_varphi_1*_u*(v_bar - curr_vel.z)/m;
    // lambda_2 = -_varphi_2*_u*(v_bar - curr_vel.z)/m;
    // if(alt <= 0.005f)
    // {
    //     lambda_1 =0.0f;
    //     lambda_2 =0.0f;
    // }
    // _rho_hat_1 += beta*lambda_1*_dt;
    // _rho_hat_2 += beta*lambda_2*_dt;
    // _rho_hat_1 = constrain_float(_rho_hat_1,0.0,0.4);
    // _rho_hat_2 = constrain_float(_rho_hat_2,0.0,0.4);

    u = u/_ahrs.cos_roll();
    u = u/_ahrs.cos_pitch();

    float thr_out;
    thr_out =u*0.001f;//+_motors.get_throttle_hover()*1428.57f;
...
float AC_PosControl::get_real_alt()
{
    float real_alt = _inav.get_altitude() - _home_alt;
    return real_alt;
}
