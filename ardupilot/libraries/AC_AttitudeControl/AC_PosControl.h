...
public:
    ...
    /// init_adaptive- initialises adaptive para before takeoff
    void init_adap();
    ...
    const float& get_rho_hat() const { return _varphi_1; }
    const float& get_h() const { return _h; }
    const float& get_vz_dot() const { return _varphi_2; }
    float get_real_alt();
    ...
    //float time_since_last_z_update() const;
...
protected:
    ...
    // Adaptive z controller
    float       _pos_z_target_last = 0.0f;
    float       _vel_z_target;          // target z velocity
    float       _vel_z_target_last = 0.0f;
    float       _accel_z_target;        // target z accel
    const float _rho_init = 0.0f;
    float       _beta_d = 0.0f;
    float       _rho_hat_1 = _rho_init;             // adaptive term
    float       _rho_hat_2 = _rho_init;
    float       _chi=0.0f;                    // integral term
    float       _chi_max = 20.0f;              
    float       _h = 0.0f;
    float       _varphi = 0.0f;
    float       _varphi_1 = 0.0f;
    float       _varphi_2 = 0.0f;
    float       _home_alt =0.0f;     // altitude at home
    bool        _home_alt_set = false;
};
